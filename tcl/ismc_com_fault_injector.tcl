...
proc process_line {line} {
    ...
    if {[llength $fields] < 2} {
        error "Line does not contain at least 2 fields: '$line'"
    }
...
############################## 故障注入 #########################
# 注入时刻
set inject_time [expr {($period*1000-$pulse_width) / 2}]
run ${inject_time}ps
...
for {set i 0} {$i < $fi_num} {incr i} {
    ...
    set fi_signal [process_line $random_line]
    echo "$i fi_signal: $fi_signal"
    set file_handle [open $fi_signal_record_filepath a]
    puts $file_handle $fi_signal
    close $file_handle
